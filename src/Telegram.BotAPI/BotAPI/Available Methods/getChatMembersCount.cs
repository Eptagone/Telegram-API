...
namespace Telegram.BotAPI.Available_Methods
{
    public static partial class AvailableMethods
    {
        ...
        /// <param name="T">BotClient</param>
        /// <param name="chat_id">Unique identifier for the target chat or username of the target supergroup (in the format @supergroupusername).</param>
        public static async Task<uint> GetChatMembersCountAsync(this BotClient T, long chat_id)
        {
            if (T == default)
                throw new System.ArgumentNullException(nameof(T));
            var stream = new MemoryStream();
            ...
            stream.Seek(0, SeekOrigin.Begin);
            return await T.RPCA<uint>("getChatMembersCount", stream).ConfigureAwait(false);
        }
        /// <summary>Use this method to get the number of members in a chat. Returns Int on success.</summary>
        /// <param name="T">BotClient</param>
        /// <param name="chat_id">Unique identifier for the target chat or username of the target supergroup (in the format @supergroupusername).</param>
        public static async Task<uint> GetChatMembersCountAsync(this BotClient T, string chat_id)
        {
            if (T == default)
                throw new System.ArgumentNullException(nameof(T));
            var stream = new MemoryStream();
