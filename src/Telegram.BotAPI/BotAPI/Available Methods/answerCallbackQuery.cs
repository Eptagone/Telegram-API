...
namespace Telegram.BotAPI.Available_Methods
{
    public static partial class AvailableMethods
    {
        ...
        public static bool AnswerCallbackQuery(this BotClient T, string callback_query_id, [Optional] string text, [Optional] bool show_alert, [Optional] string url, [Optional] uint cache_time)
        {
            if (T == default)
                throw new ArgumentNullException(nameof(T));
        ...
        public static async Task<bool> AnswerCallbackQueryAsync(this BotClient T, AnswerCallbackQueryArgs args, [Optional] CancellationToken cancellationToken)
        {
            if (T == default)
                throw new ArgumentNullException(nameof(T));
            if (args == default)
        ...
        /// <param name="cache_time">Optional. The maximum amount of time in seconds that the result of the callback query may be cached client-side. Telegram apps will support caching starting in version 3.14. Defaults to 0.</param>
        /// <param name="cancellationToken">The cancellation token to cancel operation.</param>
        public static async Task<bool> AnswerCallbackQueryAsync(this BotClient T, string callback_query_id, [Optional] string text, [Optional] bool show_alert, [Optional] string url, [Optional] uint cache_time, [Optional] CancellationToken cancellationToken)
        {
            if (T == default)
                throw new ArgumentNullException(nameof(T));
            var stream = new MemoryStream();
