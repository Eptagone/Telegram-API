...
namespace TelegramAPI.Payments
{
    public static partial class PaymentsMethods
    {
        ...
        /// <returns></returns>
        public static Available_Types.Message AnswerShippingQuery(this BotClient T, string shipping_query_id, bool ok, [Optional] ShippingOption[] shipping_options, [Optional] string error_message)
        {
            if (T == default)
                throw new System.ArgumentNullException(nameof(T));
            return T.RPC<Available_Types.Message>("answerShippingQuery", new JObject { new JProperty("shipping_query_id", shipping_query_id), new JProperty("ok", ok), ok ? new JProperty("shipping_options", shipping_options) : new JProperty("error_message", error_message) });
        }
    }
}
