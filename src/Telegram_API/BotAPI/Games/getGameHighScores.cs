...
namespace TelegramAPI.Games
{
    ...
    public static partial class GamesMethods
    {
        ...
        ///<param name="T">BotClient</param>
        /// <param name="args">Parameters.</param>
        public static async Task<GameHighScore[]> GetGameScoreAsync(this BotClient T, GetGameScoreArgs args)
        {
            if (T == default)
                throw new System.ArgumentNullException(nameof(T));
            return await T.RPCA<GameHighScore[]>("getGameHighScores", args).ConfigureAwait(true);
        }
    }
}
